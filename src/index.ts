 ...
 */
export const SECRET_PATTERNS: Array<{ pattern: string; replace: string }> = [
...
export function apply(ctx: HarnessContext, config: Partial<PluginConfig> = {}): void {
  ...
  const push = (session: SessionLike, overrides: Partial<PluginConfig> = {}): Promise<void> => {
    ...
    const effective = overrides && Object.keys(overrides).length > 0
      ...
      : cfg;
    const job = doPush(ctx, effective, session, makeRedactor(effective.redact, effective.redactSecrets)).catch((error: unknown) => {
...
function uploadArgs(cfg: PluginConfig, repo: string, file: string, pathInRepo: string): string[] {
 ...
 */
function uploadEnv(cfg: PluginConfig): NodeJS.ProcessEnv {
...
async function uploadWithRepoCreate(ctx: HarnessContext, cfg: PluginConfig, repo: string, file: string, pathInRepo: string): Promise<void> {
...
async function whoamiName(token: string): Promise<string> {
...
async function resolveRepo(cfg: PluginConfig): Promise<string> {
...
async function ensureRepo(cfg: PluginConfig, repo: string): Promise<void> {
  const token = resolveToken(cfg);
  if (!token) {
  ...
  if (owner !== username) body.organization = owner;
...
export function makeRedactor(
  rules: PluginConfig["redact"],
  includeSecrets = true,
): ((text: string) => string) | undefined {
  const sources = includeSecrets ? [...SECRET_PATTERNS, ...(rules ?? [])] : (rules ?? []);
  if (sources.length === 0) return undefined;
  const compiled: Array<{ re: RegExp; replace: string }> = [];
  for (const rule of sources) {
    ...
    try {
      ...
      throw new Error(
        `session-export-hub: invalid redact pattern ${JSON.stringify(rule.pattern)}: ${(error as Error).message}`,
      );
    }
